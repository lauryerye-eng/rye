    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <select
          ...
          className="bg-neutral-800 border border-white/10 text-neutral-300 text-sm rounded-lg px-3 py-1.5"
        >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start gap-2 flex-wrap">
                    <span className={`text-sm font-medium text-white ${a.status === "completed" ? "line-through" : ""}`}>
                      {a.title}
                    </span>
                  ...
                  {a.description && (
                    <p className="text-xs text-neutral-400 mt-0.5 truncate">{a.description}</p>
                  )}
                  <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                    <span className={`text-xs font-medium ${c.text}`}>{subject?.name ?? "—"}</span>
                    <span className={`text-xs font-semibold ${
                      a.status === "completed" ? "text-neutral-500" :
                ...
                <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-white/5 text-neutral-400 hover:text-white transition-colors text-sm"
                  >
                    ✎
                  </button>
                  <button
                    onClick={() => deleteAssignment(a.id)}
                    className="p-1.5 rounded-lg hover:bg-rose-600/20 text-neutral-400 hover:text-rose-400 transition-colors text-sm"
                  >
                    ✕
                  </button>
                </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-neutral-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <h2 className="font-bold text-white">{assignment ? "Edit Assignment" : "New Assignment"}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {form.status === "completed" && (
            <div>
              <label className="block text-xs font-medium text-neutral-400 mb-1.5">Score (0–100)</label>
              <input
                type="number"
                min={0} max={100}
                value={form.score ?? ""}
